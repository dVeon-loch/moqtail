...
# Optional: R2_PREFIX (default: $HOSTNAME), R2_CERT_FILE (default: fullchain.pem), R2_KEY_FILE (default: privkey.pem)
if [ -n "$R2_ACCOUNT_ID" ] && [ -n "$R2_ACCESS_KEY_ID" ] && [ -n "$R2_SECRET_ACCESS_KEY" ] && [ -n "$R2_BUCKET_NAME" ]; then
    echo "entrypoint: entering R2 block"
    CERT_MOUNT="/mnt/r2"
    mkdir -p "$CERT_MOUNT"

    R2_PREFIX="${R2_PREFIX:-${HOSTNAME}}"
...
EOF
    ...
    CERT="${CERT_MOUNT}/${R2_PREFIX}/${R2_CERT_FILE:-fullchain.pem}"
    KEY="${CERT_MOUNT}/${R2_PREFIX}/${R2_KEY_FILE:-privkey.pem}"
    ...
    if [ ! -f "$CERT" ] || [ ! -f "$KEY" ]; then
        ...
        if [ -n "$VOLUME_CERT_DIR" ] && [ -f "${VOLUME_CERT_DIR}/fullchain.pem" ] && [ -f "${VOLUME_CERT_DIR}/privkey.pem" ]; then
            ...
            rclone copyto -L "${VOLUME_CERT_DIR}/fullchain.pem" "r2:${R2_BUCKET_NAME}/${R2_PREFIX}/${R2_CERT_FILE:-fullchain.pem}"
            rclone copyto -L "${VOLUME_CERT_DIR}/privkey.pem"   "r2:${R2_BUCKET_NAME}/${R2_PREFIX}/${R2_KEY_FILE:-privkey.pem}"
    ...
    if [ -n "$CERTBOT_DOMAIN" ] && [ -n "$CERTBOT_EMAIL" ]; then
        ...
        if [ ! -f "$CERT" ] || ! openssl x509 -checkend "$RENEW_THRESHOLD" -noout -in "$CERT" 2>/dev/null; then
            ...
            rclone copyto -L "${CERTBOT_DIR}/live/${CERTBOT_DOMAIN}/fullchain.pem" "r2:${R2_BUCKET_NAME}/${R2_PREFIX}/${R2_CERT_FILE:-fullchain.pem}"
            rclone copyto -L "${CERTBOT_DIR}/live/${CERTBOT_DOMAIN}/privkey.pem"   "r2:${R2_BUCKET_NAME}/${R2_PREFIX}/${R2_KEY_FILE:-privkey.pem}"
...
echo "Starting relay..."
exec relay \
    --cert-file "${CERT:-/certs/certificate.pem}" \
    --key-file "${KEY:-/certs/certificate.key}" \
